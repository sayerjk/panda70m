...
def main():
    ...
    output_filename = correlation_config["base_statistics_output_filename"]
    ...
    first_moment_accumulator = DistributedStatisticsAccumulator()
    second_moment_accumulator = DistributedStatisticsAccumulator()
    ...
    with tqdm.tqdm(total=minimum_videos, disable=(dist.get_rank() != 0)) as pbar:
        for current_batch in tqdm.tqdm(dataloader, disable=(dist.get_rank() != 0)):
            ...
            # Moves the channels in the last dimension
            permuted_images = images.moveaxis(2, -1)
            permuted_images_second_moment = permuted_images ** 2

            # Accumulates the statistics
            first_moment_accumulator.accumulate(permuted_images)
            second_moment_accumulator.accumulate(permuted_images_second_moment)
    ...
    first_moment = first_moment_accumulator.compute_statistics()
    second_moment = second_moment_accumulator.compute_statistics()

    mean = first_moment.cpu().numpy()
    variance = second_moment.cpu().numpy() - mean ** 2
    ...
    if dist.get_rank() == 0:
        print_r0("- Saving base statistics to {}".format(output_filename))
